...
import type { SignalMessage, PublisherInfo } from '../infrastructure/signaling/index.ts'
...
export function ControlCenterPage() {
  ...
  const markOffline = useCallback((clientId: string) => {
    setTiles((prev) => {
      ...
      if (!tile) return prev
      return { ...prev, [clientId]: { ...tile, status: 'offline', stream: null } }
    })
  }, [])
  ...
  const startOffer = useCallback(async (publisherId: string) => {
  ...
  const handleMessage = useCallback(async (msg: SignalMessage) => {
    switch (msg.type) {
      ...
      case 'SDP_ANSWER': {
        ...
        if (pc) await pc.setRemoteDescription({ type: 'answer', sdp: msg.payload.sdp })
        break
      }
      case 'ICE_CANDIDATE': {
        if (msg.to !== viewerId) break
  ...
  useEffect(() => {
    const signaling = new SignalingClient({
      ...
      onOpen: () => {
        ...
        signaling.send(createHelloMessage(ROOM_ID, viewerId, 'viewer', 'Control Center'))
      },
    ...
    <div className="control-center">
      ...
      {tileList.length === 0 ? (
        <div className="empty-state">
          ...
          <p className="hint">
            Open <a href="/mediapipe.html">/mediapipe.html</a> on another device,
            start a task, and enable broadcasting.
          </p>
        </div>
      ) : (
